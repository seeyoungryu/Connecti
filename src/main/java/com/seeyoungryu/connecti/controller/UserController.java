...
public class UserController {
    ...
    // 알람 목록 조회 (기존 UserService → AlarmService 사용)
    @GetMapping("/alarms")
    public ApiResponse<Page<AlarmEntity>> getAlarms(Pageable pageable, Authentication authentication) {
        Page<AlarmEntity> alarms = alarmService.getAlarms(authentication.getName(), pageable);
        return ApiResponse.success(alarms);
    }

    // 알람 읽음 처리 (기존 UserService → AlarmService 사용)
    @PatchMapping("/alarms/{alarmId}/read")
    public ApiResponse<Void> markAlarmAsRead(@PathVariable Long alarmId, Authentication authentication) {
        alarmService.markAlarmAsRead(alarmId, authentication.getName());
        return ApiResponse.success(null);
    }
}
